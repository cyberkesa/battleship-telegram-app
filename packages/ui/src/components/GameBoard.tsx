...
// Функция для проверки валидности размещения (локальный fallback, если понадобится)
...
export const GameBoard = forwardRef<GameBoardHandle, GameBoardProps>(({...props}, ref) => {
	...
	// Внутренние преобразователи в формат game-logic
	const toFleet = useCallback((ships: PlacedShip[]) => {
		return ships.map(s => {
	...
	const getCellState = useCallback((x: number, y: number): CellState => {
		...
		if (previewShip) {
			...
			if (isUnderPreview) {
				return previewShip.isValid ? 'hover' : 'invalid';
			}
		}
